...
- **[CV0.E4.S9 — Sidebar 'Conversations' entry](docs/project/roadmap/cv0-foundation/cv0-e4-home-navigation/)** (shipped 2026-04-22) — sidebar gains `Conversations` (plural) link to `/conversations`, alongside the existing `Conversation` (singular) which continues to drop into the active session. `.sidebar-link--secondary` styling differentiates the listing from the primary chat action.
