...
- **[CV1.E6.S1 — Conversations browse](docs/project/roadmap/cv1-depth/cv1-e6-memory-map/cv1-e6-s1-conversations-browse/)** (shipped 2026-04-22) — new `/conversations` cross-scope surface with filters by persona / organization / journey via query string, recency sort, paginated (50/page, "Show N more"). The active session gets a "current" badge in the list. Sibling stories pending: sidebar entry "Conversations" (CV0.E4.S9) and scope ateliê trim with "View all (N)" link (CV1.E4.S5 follow-up). First concrete piece of CV1.E6 (Memory Map), promoted ahead of the original landing-first plan after S5 surfaced the cross-scope need. New `getConversationsList` helper; new `ConversationsListPage` component. 19 new tests.
