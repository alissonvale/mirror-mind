...
| # | Story | Status | Notes |
|---|---|---|---|
| S1 | [Chrome inversion + page skeleton](cv1-e12-s1-chrome/) | ⏳ Drafted | Logo → `/espelho`. New `Iniciar` pill. Empty `/espelho` shell with TopBarLayout. Tests for both navigation paths. |
| S2 | [Living synthesis: glance + pulse + depth](cv1-e12-s2-synthesis/) | ⏳ Drafted | The actual content — the three panes synthesized from cognitive/territory/memory state, plus glance + "what shifted" pulse. |
