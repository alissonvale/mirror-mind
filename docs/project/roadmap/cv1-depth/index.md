...
- [CV1.E13 — Portraits](cv1-e13-portraits/) — read view for orgs, journeys, and scenes. Every entity link landing on a CRUD form conflates stewardship with reading; this epic extends the `/espelho` metabolism to each territory. **S1 (journey portrait) shipped end-to-end 2026-05-04** in five rounds (URL migration + skeleton, deterministic synthesis core, conversations listing, LLM citable lines via `entity_profile_cache`, reference-draft reproduction tests). S2 (org) and S3 (scene) queued. [Design](../../../design/entity-profiles.md).
