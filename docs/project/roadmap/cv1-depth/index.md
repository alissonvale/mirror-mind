...
- [CV1.E12 — The Mirror page (`/espelho`)](cv1-e12-espelho/) ✏️ Drafted 2026-05-03 — separates the brand mark from the operational entry. Logo `◆ Mirror Mind` flips to point at a new contemplative surface `/espelho` that synthesizes who the user is, where they operate, and what they lived. New `▶ Iniciar` pill takes over the operational job the logo used to carry. Three stories: chrome inversion (S1), living synthesis with glance + pulse + depth (S2), pinned inscriptions (S3).
