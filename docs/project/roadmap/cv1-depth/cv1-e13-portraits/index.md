...
**Status:** ✅ Done (extended). S1 + S2 shipped 2026-05-04, S3 + S4 on 2026-05-05.
...
| [`CV1.E13.S3`](cv1-e13-s3-scene-portrait/) | **Scene portrait** ✅ | Shipped 2026-05-05. Briefing IS the lede (no diagnosis-extraction). "QUANDO ELA ACONTECE" + "ELENCO" (with voice bifurcation: persona list vs ♔ Voz da Alma) + "EM QUAL TERRITÓRIO". Empty briefing renders stub block with voice glyph in display position. Plum accent. |
| [`CV1.E13.S4`](cv1-e13-s4-persona-portrait/) | **Persona portrait** ✅ | Shipped 2026-05-05. Lede is the first paragraph of authored content. "ONDE ELA APARECE" lists journeys + scenes where the voice is active. Two new editorial sections: **POSTURA** (extracted from `## Postura`, rendered as flowing prose) + **ANTI-PADRÕES** (extracted from `## Anti-padrões`, rendered as a deliberately austere bullet list). Accent color is **per-row** (`identity.color`) — first portrait whose accent isn't axis-fixed. Workshop form remains at `/map/persona/<key>` (legacy); `/personas/<key>/{editar,edit}` redirects there. |
