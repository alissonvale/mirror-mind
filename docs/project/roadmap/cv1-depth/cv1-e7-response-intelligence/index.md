...
| S9 | **Per-turn mode visibility** — stamp `_mode` and `_mode_source` (reception vs session-override) on the assistant entry meta when persisting; surface the mode in the `Look inside` composed snapshot per-turn; optional subtle visual indicator on the bubble (border accent / microtext). Today mode is decided fresh by reception every turn (or fixed by a session override), but the decision is invisible after the fact — users can't verify retroactively which mode reception chose for any given turn. The transparency closes the loop on the pipeline's per-turn decisions, mirroring the snapshot's existing role for personas/scopes | Draft |
