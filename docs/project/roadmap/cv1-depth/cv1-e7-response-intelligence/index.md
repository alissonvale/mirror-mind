...
**Status:** S1 ✅ · S2 ✅ · S3 ✅ · S4 ✅ · S5 ✅ · S8 ✅ · S9 ✅ · S6–S7 drafts · S5b follow-up queued
...
| [S8](cv1-e7-s8-out-of-pool-rail-suggestion/) | **Out-of-pool suggestion via the rail** — reception emits "would have picked" candidates that the session pool filtered out; the rail surfaces a non-modal suggestion (*"`maker` may have something to say. [Hear it]"* / *"Add `vida-economica` context. [Yes]"*); on click, a divergent one-turn response runs through that persona/scope and renders inline as a sub-bubble with its badge and color, **without modifying the session pool**. New `divergent_runs` table; new `POST /conversation/divergent-run` endpoint. Resolves the cast-vs-scope tension surfaced during S3 manual smoke. | ✅ Done |
