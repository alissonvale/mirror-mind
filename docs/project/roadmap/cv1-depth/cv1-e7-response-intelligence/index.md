...
**Status:** S1 ✅ · S2 ✅ · S3 ✅ · S4 ✅ · S5 ✅ · S9 ✅ · S6–S8 drafts · S5b follow-up queued
...
| [S4](cv1-e7-s4-conditional-identity/) | **Conditional identity layers** — `self/soul` and `ego/identity` compose only when reception flags the turn as touching identity / purpose / values. New `touches_identity: boolean` axis on reception; identity-conservative defaults (silence skips both layers); composer gates the pair together (single boolean, not split). | ✅ Done |
...
| S8 | **Out-of-pool suggestion via the rail** — reception emits "would have picked" candidates that the session pool filtered out; the rail surfaces a non-modal suggestion (*"`tecnica` may have something to say. Hear it?"* / *"Add `vida-economica` context to this answer?"*); on click, a divergent one-turn response runs through that persona/scope and renders inline with its badge and color, **without modifying the session pool**. Resolves the cast-vs-scope tension surfaced during S3 manual smoke (cast supposedly mutable, but pool-filter blocks auto-growth). See [prompt-composition lifecycle § Parked alternatives](../../../../product/prompt-composition/index.md#parked-alternatives--three-design-points) | Draft |
| S9 | **Per-turn mode visibility** — stamp `_mode` and `_mode_source` (reception vs session-override) on the assistant entry meta when persisting; surface the mode in the `Look inside` composed snapshot per-turn; subtle bubble glyph. Today mode is decided fresh by reception every turn (or fixed by a session override), but the decision was invisible after the fact — users couldn't verify retroactively which mode reception chose for any given turn. The transparency closes the loop on the pipeline's per-turn decisions, mirroring the snapshot's existing role for personas/scopes. **Phase 1 shipped** as a refinement (stamping + bubble glyph; see [bubble-metadata-legibility improvement](../../improvements/bubble-metadata-legibility/)). **Phase 2 shipped** — `mode:` row in the Composed section of the rail's Look inside, reading the same `_mode` field stamped in phase 1. | ✅ Done |

**Ordering rationale:** S1 is the tracer bullet for the pipeline pattern — `ego/expression` is the cheapest layer to peel off first. S2 is the **surface** follow-up that made itself necessary during S1's use: the rail-as-junk-drawer critique surfaced, and the cast-vs-scope insight came from watching conversations populate. S2 reshapes the UI to match the real data model (personas are a mutable ensemble; orgs/journeys are stable context) and installs the visual scaffolding S5 will plug into. S3–S5 peel further layers off the prompt one at a time. S6 re-wires retrieval and unlocks attachments. S7 is the abstraction payoff, deferred until there's enough shape to abstract honestly. S8 is the rail-driven affordance for crossing the pool boundary without committing to it — a direct outgrowth of the friction S3 surfaced (and the inverse of S3's tightening: S3 closes leaks; S8 opens an explicit one-turn door). S9 closes the transparency loop on mode — currently the only pipeline axis whose per-turn decision is invisible after generation; a small, surgical addition to the existing snapshot pattern.
