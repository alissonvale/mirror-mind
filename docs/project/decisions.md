...
**Why doctrine ships from Alisson's seed by default.** The cena pivot's design explicitly chose this for the household-adoption phase: Veronica receives concrete substance to appropriate or edit instead of a blank slate, and the Alma compose path lands on real principles from turn 1 instead of a thin "tell me about yourself" dance. The honest tradeoff — a third party would get Alisson's personal philosophy as their starting point — is acknowledged and addressed by the already-backlogged `--seed` flag (task `76efa059`) that future provisioning will use when adoption widens.

**Why Voz da Alma cena with empty briefing instead of a richer template.** Three options were considered: (1) ship with rich, opinionated briefing (the o-espelho's conversational prompt verbatim), (2) ship with an empty briefing, (3) ship with no Voz da Alma cena at all and let the user create it. Option 1 conflates the cena's briefing with the Alma compose path's substance — Alma already gets soul + doctrine + identity through its preamble, so the cena would be doubly-loaded. Option 3 leaves `/inicio` with a "Nova cena" card and nothing else — the empty home that the design explicitly aimed to prevent. Option 2 (chosen) gives the user a clickable card from day one without putting words in their mouth; the briefing field is theirs to fill (or leave) when they edit the cena.

**Why `provisionUser` is pure (throws, doesn't `process.exit`).** Two consumers want different behavior: the CLI wants `process.exit(1)` on conflict, tests want assertable exceptions. The pure function returns `ProvisionResult` and throws on duplicate; the CLI handler wraps it for the exit + console output. Lets `tests/onboarding-seed.test.ts` call the helper directly without intercepting `process.exit` or stubbing console.

**Why cutover stops at the redirect, not a sidebar-removal sweep.** The locked design said "small PR" for S5. Migrating each entity workshop to TopBarLayout chrome is a real refactor — every page (`/personas`, `/organizations`, `/journeys`, `/conversation`, `/me`, `/map`) needs its layout swap, sidebar references removed from helpers, all with test coverage updates. Bundling that in S5 would inflate the PR significantly and risk regressions far from the cena pivot's substance. Each surface migrates on its own beat when there's a reason to touch it; the strangler keeps the old chrome visible during the transition.

**Why `/_legacy-home` exists.** One-release safety net. The HomePage is the most-tested page in the codebase; killing it the same day we redirect away from it deletes a lot of validated logic without a probationary period. Two release cycles later, if `/_legacy-home` is still unaccessed (no analytics show traffic), it gets removed.
