...
Current focus: **CV1.E8.S1 LLM call logging — released as `v0.19.0` (2026-04-27).** Every model invocation across the pipeline (reception, main, expression, title, summary) now writes a row to `llm_calls` with the full system prompt, user message, response, tokens, cost, latency, session_id, entry_id, env, error. Admin UI at `/admin/llm-logs` surfaces the rows with filters (role, session, model, search-in-prompt), pagination, a toggle for the global `llm_logging_enabled` setting, cleanup actions (clear all + clear older than N days), and JSON/CSV export with RFC 4180 escaping. Detail view renders system_prompt and response in monospace `<pre>` blocks with `white-space: pre-wrap` so admin reads what was actually sent and received — no markdown render, no whitespace collapse. Bilingual chrome (en + pt-BR). Defensive throughout — `logLlmCall` never throws, never blocks the pipeline. The substrate the next week of Alma refinements needs: every "the response read flat" complaint can now be traced to the exact prompt that produced it.
