...
Current focus: **CV2.E1 Localization — S1, S3, S4 shipped; S2 partial** (2026-04-27). The localization rail is live end-to-end: a Brazilian user picks "Português (Brasil)" in `/me`, and the sidebar, login, home, `/me`, `/conversation` (cast + context + mode + menu + Look-inside rail + composed-prompt drawer) and `/conversations` browse render in pt-BR. S2's remaining surfaces — organizations, journeys, map, layer-workshop, personas, docs, admin/* — are still hardcoded English; they translate in a follow-up round. S5 (Brazilian narrative tenant) is open and needs a persona alignment with the user before authoring.
