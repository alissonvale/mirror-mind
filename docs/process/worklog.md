...
**CV1.E15.S7 — rewritten same-day from divergence-based to per-session toggle (2026-05-05).** First take rendered the bubble badge whenever a turn's stamped `_model_id` differed from the session's currently-resolved model. Rule was hard to predict in practice (badge appeared and disappeared as the resolver chain shifted under the admin's feet) and the Look-inside rail's "model" row still claimed a single model — wrong when the session mixed. Rewrote: a binary per-session toggle (`sessions.show_model_badges`, default 0) inside the Advanced pouch (admin-only segmented `ocultar/mostrar`). Off → no badges anywhere. On → every assistant turn with stamped `_model_id` shows `⊕ <model_short>`. Persisted per-session — admin enables in the conversation where they're comparing models, doesn't leak to others. Toggle save = native form submit + reload (no async path; `/conversation/show-model-badges` POST is admin-gated, 403 for non-admin). The `done` SSE event now ships `assistantModel: { provider, id }` so streamed bubbles paint the badge live when the toggle is on at send time. In parallel: `SessionStats.model: string` retired in favor of `models: Array<{ model_id, count }>` aggregated from `_model_id` meta via `GROUP BY`; the rail's Composto/model row renders `gemini-2.5-flash ×7, claude-sonnet-4-6 ×2` (sorted by count desc). `MirrorPage.currentMainModel` prop and the `resolveMainModel` pre-compute in the GET handlers retire — the toggle replaces the divergence comparison. New i18n: `header.modelBadges.{label,off,on}`. **7 new tests** (`tests/show-model-badges-toggle.test.ts`): helper round-trip, route admin/non-admin, session-stats aggregation + pre-S4 skip. Existing `session-stats.test.ts` updated. 1248/1248 suite green. **Doc note**: the S7 folder slug stays `cv1-e15-s7-divergence-badge` for git history continuity even though the story is no longer about divergence.

**CV1.E15 epic closed (2026-05-05) — S5 + S6 + S7 in one round.** Three stories that close the per-turn loop: (1) **S5**: the per-turn `×` becomes a `⋯` kebab for admins, with a panel listing **Re-executar com modelo…** + **Excluir**; non-admins keep the bare `×`. Single `<div id="rerun-popover">` rendered once per page (admin only), opened by `[data-rerun-trigger]` clicks across any bubble — provider input + datalist-backed model picker + Save/Cancel + status line. Inline JS in `mirror.tsx` wires open / close / submit / reload. (2) **S6**: `POST /conversation/turn/rerun` (admin-only) — replays the user message that produced an assistant entry through a different model, **mutates the assistant entry in place** (same id, new content, new `_model_provider/_model_id`, plus `_rerun_at`). Inherits persona / scope / mode / length / Alma flag from the parent meta — only the model changes. Reception is NOT re-run. Guards: 403 non-admin, 400 missing fields, 404 entry not found, 403 foreign user, 400 non-assistant, 400 trivial-stamped, 400 orphan assistant. `logLlmCall` writes against the same `entry_id` for trace. (3) **S7**: bubble badge `⊕ <model_short>` appears on assistant turns whose stamped `_model_id` differs from the session's currently-resolved model. `MirrorPage` accepts `currentMainModel`; both GET handlers pre-compute via `resolveMainModel` and pass through. Tooltip on the badge shows the full model_id. Falsy / missing values short-circuit to no badge — turns stamped before S4 are silent. New CSS for `.msg-actions*` + `.rerun-popover*` + `.msg-badge-model`; `style.css?v` bumped to `turn-actions-1`. New i18n keys for the menu, popover, and rerun status copy. **7 new tests** (`tests/turn-rerun.test.ts`) covering the validation surface (auth, body, ownership, entry-type guards). Happy path is manual — covered in the manual roteiro. 1232/1232 suite green. **Epic CV1.E15 closes** — the admin can now choose a model per scene (S2), per session (S3), or per individual turn (S5+S6), with the resolver chain (S4) honoring the precedence and the badge (S7) making divergences visible. Picker (S1) is reused across all four placement points.

**CV1.E15.S4 — Resolver + per-turn stamping shipped (2026-05-05).** The keystone — turns now honor session/scene overrides and every assistant entry stamps the model that produced it. New `server/main-model-resolver.ts` with `resolveMainModel(db, sessionId, userId)` returning `{ provider, model, source, globalConfig }`. Precedence: session.{model_provider,model_id} → scene.{model_provider,model_id} → models[role='main']; each tier needs BOTH fields, partial values fall through. All four main paths swapped `getModels(db).main` for the resolver: API adapter (`server/index.tsx`), web SSE main streaming (`adapters/web/index.tsx`), divergent runs (`adapters/web/index.tsx`), telegram (`adapters/telegram/index.ts`). Every assistant entry now stamps `_model_provider` + `_model_id` in meta — `/admin/llm-logs` shows the actual model that ran each turn. **Auth handling**: `resolveApiKey(db, "main")` still routes through the global role, which works for the common case (admin pinning a different OpenRouter model) but breaks for cross-provider overrides — documented as follow-up. **session-stats** still uses global for cost estimate display in the rail — minor inaccuracy, doesn't affect generation, deferred. **7 new tests** (`tests/main-model-resolver.test.ts`): no override → global, session wins, session partial falls through, scene wins, session beats scene, scene partial falls through, globalConfig always populated. 1225/1225 suite green. **Behavior change**: from this commit on, S2's per-scene and S3's per-session overrides effectively determine which model responds. **Next**: S5 + S6 (per-turn kebab menu + destructive rerun endpoint) + S7 (badge for divergent turns).

**CV1.E15.S3 — Per-session model override shipped (2026-05-05).** Schema + helpers + admin-only header surface + POST handler. Two new columns on `sessions` (`model_provider`, `model_id`, both nullable). New `getSessionModel` / `setSessionModel` in `server/db/sessions.ts` (with empty-string→NULL normalization and ownership-enforced UPDATE). New `SessionModelState` interface in context-rail.tsx; `RailState.sessionModel` populated by `buildRailState`. The conversation header's `<details class="cena-advanced">` pouch gains a "Modelo" row inside `AdvancedZone`, **rendered only when isAdmin && modelCatalog**, with provider input + S1's `<ModelPicker>` + Save button + applies-from-next-turn hint. Pouch summary now appends `· <model_short>` when an override is set (last `/`-segment of model_id, so multi-vendor IDs don't blow up the pill width). `MirrorPage` and both GET handlers (`/conversation` and `/conversation/:sessionId`) updated to load the catalog server-side when `user.role === "admin"`. The async `refreshConversationHeader` path inherits this for free since it re-fetches the same GET. New route `POST /conversation/model` 403s for non-admin (defense in depth — admin gating happens at render *and* route level). New CSS rules for `.header-advanced-row-model` + form children; `style.css?v` bumped to `session-model-row-1`. New i18n keys: `header.model.{label,providerPlaceholder,save,hint}`. **9 new tests** (`tests/session-model.test.ts`): helper-level (null default, persists, empty→NULL, ownership), route-level (admin persists, admin clears, non-admin 403), render-level (admin sees the row, non-admin doesn't). 1218/1218 suite green. **Resolver consumption deferred to S4** — until then the persisted fields exist but don't affect generation. **Next**: S4 (resolver + per-turn stamping in main path) — the keystone that makes S2 + S3 actually do something.

**CV1.E15.S2 — Per-scene model override shipped (2026-05-05).** Schema + form + handlers, admin-gated. Two new columns on `scenes` (`model_provider`, `model_id`, both nullable — NULL = inherit global main). `Scene` interface, `CreateSceneFields`, `UpdateSceneFields`, `rowToScene`, `createScene`, `updateScene` all extended; `normalizeNullable` helper collapses empty strings to NULL so the form's "" cleared-field convention round-trips. Form gains a model block inside `<details class="cena-advanced">`, **rendered only for `user.role === "admin"`**, with the provider input + S1's `<ModelPicker>`. Route handlers (`POST /cenas/nova` and `POST /cenas/:key/editar`) parse the fields unconditionally (shape symmetry) but only forward to `createScene`/`updateScene` when the requester is admin — defense in depth: a non-admin POSTing model fields can't disturb admin-set values. New i18n keys: `scenes.form.advanced.{modelProvider.label,modelProvider.placeholder,modelId.label,modelHint}`. **9 new tests** (`tests/cenas-model.test.ts` — CRUD with model fields, empty-string→NULL normalization, admin form renders the block, non-admin form doesn't, admin POST persists, non-admin POST ignored, non-admin POST preserves admin-set values). 1209/1209 suite green. **Resolver consumption deferred to S4** — until then the columns persist but don't affect generation. **Next**: S3 (per-session override in the chat header) + S4 (resolver + per-turn stamping). S4 is the keystone for the whole user-visible behavior — until it lands, S2 and S3 are just storage.

**CV1.E15.S1 — Model catalog + picker shipped (2026-05-05).** New epic kicked off in the same v0.24.0 cycle. The driving frustration: `/admin/models` makes the admin type model IDs from memory; typos silently break the role until the next request fails. S1 builds the substrate that S2-S7 will reuse — a live catalog (OpenRouter `/v1/models` + curated extension via `config/models-catalog.json`) and a reusable `<ModelPicker>` combobox. `getCatalog(db, opts)` caches in-memory (TTL 1h, `?refresh=1` bypass), gracefully degrades when `OPENROUTER_API_KEY` is missing or the network is down (curated only, no throw). Prices auto-convert USD→BRL via the existing `usd_to_brl_rate` setting. New admin route `GET /admin/models/catalog` exposes the merged list as JSON for future client-side picker hydration; `/admin/models` itself loads the catalog server-side and feeds the picker, so the input field gains autocomplete with no JS change. The picker is server-rendered (`<input list> + <datalist>`) — accepts free typing for power admins and dropdown discovery for new admins. Curated file ships empty (admin populates as needed); shape documented inline. **Decision recorded**: this epic supersedes CV1.E8.S2 (the side-by-side divergent-runs variant of per-turn switching) — destructive rerun is the better fit for the actual use case. New i18n key `admin.models.catalogEmpty`. **Tests**: 6 unit (`tests/models-catalog.test.ts` — happy path, cache, force refresh, network failure, missing key, provider filter) all passing. Smoke + full suite stay green at 1200/1200. **Next**: S2 (per-scene model override) — schema columns on `scenes`, picker in scene form, resolver respects scene as fallback ahead of global.

**CV1.E14 — Identidade rename + section relabel (2026-05-05).** Single follow-up pass right after CV1.E14's initial ship. The page first shipped as `/narrativa` with section "IDENTIDADE"; the naming had a collision (the section labeled the same word as the page felt redundant) and the user rerouted: page becomes **Identidade** / **Identity**, section becomes **PAPEL** / **ROLE**. Mechanical rename touched route URLs (`/narrativa|narrative` → `/identidade|identity`), file names (`narrativa-synthesis.ts` → `identidade-synthesis.ts`, `narrativa.tsx` → `identidade.tsx`), function/type names (`composeNarrativa` → `composeIdentidade`, `NarrativaState` → `IdentidadeState`), state field `identidade` → `papel`, CSS class prefixes (`.narrativa-*` → `.identidade-*`), i18n key namespace (`narrativa.*` → `identidade.*` with `.section.identidade`/`.stub.identidade` → `.section.papel`/`.stub.papel`), and chrome links across `/espelho` Sou pane + avatar dropdown. Page title flips pt-BR/en accordingly. Suite stays at 1193/1193 — five existing tests updated for the new chrome strings + URL; `tests/identidade.test.ts` rewritten with new symbol names.

**CV1.E14 — Identity Narrative shipped (2026-05-05).** Brand-new epic, single story. The cognitive-map metaphor (`/map` as a structural grid of layer cards) gives way to `/narrativa` — a continuous-read self-portrait that tells the user's identity as memoir instead of laying it out as a diagram. Five flat sections (ALMA, IDENTIDADE, COMPORTAMENTO, EXPRESSÃO, ELENCO) with no "ego" agrupador — per the 2026-05-05 directive that "ego" leaves the user-facing chrome. Sub-headings within each layer render as `☞ heading` in italic serif (typographic finger pointing into the next chapter, not card-breaking h3). Single edit affordance in the footer ("ir ao mapa para editar") cycles back to `/map` workshop home where layer-by-layer editing happens. Personas list in ELENCO with each name linking to its portrait (S4 destination consumed). **Light synthesis** — no LLM. `parseLayer` strips the leading H1, captures preamble paragraphs, splits the rest into named subsections by `## ` markers; renders verbatim. Bookplate at top mirrors `/espelho`. Locale-aware route (`/narrativa` pt-BR + `/narrative` en alias via Hono regex). Avatar dropdown's "Mapa Interior"/"Inner Map" flips to "Narrativa"/"Narrative" pointing at `/narrativa`; `/espelho` Sou pane drills here too. `/map` itself stays rendering the legacy cognitive-map dashboard for back-compat with admin flows + existing internal links — the chrome no longer surfaces it as canonical, but the page exists. 1193/1193 tests passing (was 1182; +11 narrativa, 0 net regressions). Audit pending: remaining "ego" labels in `/map` legacy page, layer breadcrumbs, and layer-workshop chrome — drop as those surfaces are touched.

**CV1.E13.S4 — Persona portrait shipped (2026-05-05).** Reopened the Portraits epic to add the fourth entity type. Single round, ~70% engine reuse. `/personas/<key>` now lands on a read view; the workshop form remains at the legacy `/map/persona/<key>` URL with `/personas/<key>/{editar,edit}` aliasing to redirect (backward-compat). Two named differences from S1-S3: (1) **per-row accent color** — `identity.color` drives the left strip + tile number color, scoped via `[data-entity="persona"]` so other portraits keep their axis-fixed accents (first portrait where the accent isn't axis-fixed); (2) **two new editorial sections** — POSTURA (extracted from `## Postura`, flowing prose) and ANTI-PADRÕES (extracted from `## Anti-padrões`, austere bullet list with persona-color marker). "ONDE ELA APARECE" surfaces journeys (via `_persona` + `_journey` meta) and scenes (via `scene_personas` junction). Lede is the first paragraph of authored content. Tiles cap at 2 (conversations through this voice + recency). Close picks the last short sentence of `## Postura`. New i18n keys: `portrait.{whereItAppears, posture, antipatterns, noConversationsYetPersona, editLink.persona}`. 1182/1182 tests passing (was 1172; +10 across content parsing, integration, route smoke). Also banked **a feedback memory**: avoid the term "Ego" in user-facing GUI from now on — "Persona" is natural and non-technical; "Ego" carries pejorative weight. Internals (DB schema `identity.layer = 'ego'`, system prompts, code variables) keep "ego"; the constraint is GUI-scoped only. Audit pending: `/map`, `/me`, and any current ego/identity / ego/behavior / ego/expression labels in the chrome.

**CV1.E13.S3 — Scene portrait shipped (2026-05-05).** Third and final story of the Portraits epic. One round; ~70% engine reuse from S1/S2. New surface at `GET /cenas/:key`; workshop form continues at `/cenas/:key/{editar,edit}` (locale-aware alias added). Anatomy diverges from S1/S2 in named ways: (1) **the briefing IS the lede** — cenas are declarative, not diagnostic, so no extraction heuristic; (2) "A pergunta viva" and structural section pulled out (cenas are unities, not decisions); (3) tiles cap at 2 (conversation count + recency) — no "tempo desde início" since cenas are operational, not biographical; (4) **voice bifurcation** in the cast section — `voice='persona'` lists cast personas with descriptors via `scene_personas` junction, `voice='alma'` renders a single ♔ Voz da Alma indicator with warm-amber glyph; (5) "QUANDO ELA ACONTECE" surfaces the `temporal_pattern` in italic; (6) "EM QUAL TERRITÓRIO" lists `organization_key` (`⌂`) and `journey_key` (`↝`) when declared. Empty briefing renders a stub block — `♔` (alma) or `❖` (persona) glyph in display position + an italic invitation to author the briefing. Visual silhouette: plum accent `#9a8ba0` (the /espelho Vivo-pane palette), scoped via `[data-entity="scene"]`. Conversations queried via `sessions.scene_id` (CV1.E11.S4) — different signal than journey/org which read `_journey`/`_organization` meta. New i18n keys: portrait.{whenItHappens, cast, castAlma, castAlmaDescriptor, inWhichTerritory, noConversationsYetScene, sceneStubAlma, sceneStubPersona}. 1171/1171 tests passing (was 1160; +11 across persona-voiced/alma-voiced cenas, territory section, tile composition, lede + close pickers). **CV1.E13 epic now has all three stories shipped** — journey, organization, and scene each have their own read-view-by-default surface; the workshop form is reachable via discreet "editar" link from each portrait.

**CV1.E13.S2 — Organization portrait shipped + width fix (2026-05-04).** Second story of the Portraits epic. Single round (the engine from S1 carried 80% of the work). `/organizations/<key>` now lands on the read view; the workshop form moved to `/organizations/<key>/{editar,edit}`. Anatomy differs from S1 in three named ways: (1) lede is **situation-first** (orgs use briefing as identity manifesto, situation as current diagnosis — flips the journey heuristic; for Pages Inteiras the lede combines situation's first paragraph with briefing's punchline closing); (2) **"Quem passa por aqui"** replaces "Onde ela mora" — orgs don't live somewhere, they're a place that hosts; section lists nested journeys with status + recency, plus the most-frequent persona and anchored scene; (3) accent palette flips to warm-amber `#b8956a` (Sou pane palette from /espelho), scoped via `[data-entity="organization"]` so journey portraits keep their teal. **Width fix shipped first**: portrait shells were 640px, the system standard is 980px (workshops, lists, /espelho). Portrait shell now 980px outer with 720px inner reading column for prose blocks (lede, sections, close). Aligned via shared `portrait-shared.tsx` extracting `<PortraitLede>`, `<NumericTilesRow>`, `<ConversationsSection>`, `<PortraitClose>`, `<PortraitFooter>`, `editPathFor`, and the `PORTRAIT_STYLES` constant — both journey and org pages render through the same module. Structural detector relaxed: front markers now accept both bold (`**A primeira é X.**`) and plain (`A primeira é uma queda...`) forms via paragraph-based parsing. Pages Inteiras's situation activates as fronts; Pós-Lançamento (which uses bold) still works. Reference draft test against Pages Inteiras's verbatim source content — lede source = situation, structural detector returns 3 frentes. 1160/1160 tests passing (was 1151; +9 across org synthesis + reference draft + cache reuse). 8 existing CV1.E4.S5 workshop tests updated to hit `/organizations/:key/edit` instead of `/organizations/:key` — same assertions, new URL.

**CV1.E13.S1 — Journey portrait shipped end-to-end (2026-05-04).** First story of the new Portraits epic — the `/journeys/<key>` URL now lands on a contemplative read view; the workshop form moved to `/journeys/<key>/editar` (locale-aware: `/edit` for `en`). All five rounds in one session: (1) URL migration + skeleton; (2) deterministic synthesis core (lede from briefing's last paragraph, tiles for tempo + structural anchor + recency, "onde ela mora" with org/persona/scene adjacencies + parenthetical for absences, structural section detector regexing scenarios vs fronts, live-question detector with confessional layer); (3) conversations listing from `getJourneySessions`; (4) LLM citable-line extraction with `entity_profile_cache` source-hash invalidation, fire-and-forget warmup pattern (sync render reads cache, background pass fills misses); (5) reference-draft reproduction tests against verbatim source content from Antonio Castro's three travessias. **The three drafts in [docs/design/entity-profiles.md](../design/entity-profiles.md) are now reproduced structurally by the synthesis pipeline** — lede source ≡ briefing for all three, scenarios pattern landed for Voltar a BH, fronts pattern for Pós-Lançamento, live-question + confessional layer captured where present. Visual smoke against a provisioned Antonio Castro tenant pending. 1151/1151 tests passing (was 1108 at start of round 1; +43 across cache, synthesis, reference drafts, route handlers). New files: `server/portraits/{journey-synthesis,cache,conversation-citable-line}.ts`, `adapters/web/pages/journey-portrait.tsx`, `tests/{journey-portrait,entity-profile-cache,journey-portrait-reference-drafts}.test.ts`. **Next**: S2 (organization portrait) reuses much of S1's plumbing; S3 (scene portrait) likely reopens design before code.

**Design lock (2026-05-04) — Entity profiles for orgs, journeys, and scenes.** Long product-designer conversation triggered by the observation that today every entity link drops into a CRUD form — that conflates stewardship with reading. The /espelho proved a different relationship is possible; this design extends it to each territory the user operates inside. Three reference drafts authored end-to-end against Antonio Castro's travessias (Bia Saturada, Voltar a BH, Pós-Lançamento), each stress-testing different shapes: rich+active, decision-structured, rich+silent. Ten consolidated principles (sections conditional on data, lede length variable, absences declared not silent, structural patterns in source produce structural sections, briefing-sourced fechos read with more weight than conversation-sourced, etc.). URL migration spec: `/<entity>/<key>` becomes the read view, `/<entity>/<key>/editar` carries the form (locale-aware). LLM extraction narrowed to two points (citable conversation line per session; lede synthesis when briefing+situation too short), both `source_hash`-cached. Implementation parked — outside the current janela (manutenção only). Conversation locked in [docs/design/entity-profiles.md](../design/entity-profiles.md) so the first construction session does not need to reconstruct it. **Sequencing when janela opens**: S0 URL migration framework, S1 travessia profile (highest-yield first surface), S2 organization profile, S3 scene profile.

**Maintenance (2026-05-04) — Narrative tenants get soul summaries + ímãs so `/espelho` reads alive.** The Antonio Castro screenshot from the espelho surfaced two gaps: (1) the SOU pane was rendering raw markdown — `# Soul ## Quem eu sou Eu sou alguém que confia...` — because `pickSummaryOrFirstSentence` falls through to the layer body when `identity.summary` is null, and the body's first sentence catches the `# Soul` header inline; (2) the inscription tile was empty (footer link only), because no narrative tenant had any seeded ímã. Fix is content-first: each of the six narrative tenants gets a `## Summary` section in `identity/self/soul.md` (one-line distilled voice, ≤200 chars) and a new `inscriptions.md` (3-5 phrases, mix of external citations + the tenant's own principles, one pinned for the tile). Loader changes: `upsertIdentity` extracts `## Summary` and persists via `setIdentitySummary` — works for any layer that opts in, not just soul. New `loadInscriptions` reads `<userDir>/inscriptions.md`, parses bullet lines (`- "text" — Author *(pinned)*`), and seeds with `createInscription` + optional `pinInscription`. **Idempotent on the user**: re-runs leave existing inscriptions alone so user customizations (pins, edits, archives) survive a re-provision. Each tenant's `index.md` updated to reference inscriptions.md. 1106/1106 tests passing (was 1099; +7 across summary plumbing, inscription seeding, idempotence, and `parseInscriptionLine` unit tests). Fresh `narrative load` (or `--reset-conversations`) refreshes session timestamps so the VIVO pane lands non-empty. **Parked**: extending `## Summary` to other layers (ego/identity, persona/*) for the Mapa Interior cards — same plumbing, separate content authoring round.

**v0.24.0 released — *The mirror reads itself* (2026-05-05).** Bundles ~25 commits since v0.23.0 into one shipping unit. Two epics: **CV1.E13 (Portraits)** with four stories — journey, organization, scene, persona — each gaining a memoir-shaped read view at `/<entity>/<key>`, with the form moving to `/<entity>/<key>/editar` (or `/edit` for `en`). Engine reuse 70-80% across stories. Persona portraits are the first with per-row accent color (`identity.color`); the cast section parses POSTURA + ANTI-PADRÕES sections from authored markdown. **CV1.E14 (Identidade)** retires the cognitive-map metaphor: `/identidade` (pt-BR canonical) + `/identity` (en alias) becomes the continuous-read self-portrait — ALMA / PAPEL / COMPORTAMENTO / EXPRESSÃO / ELENCO, with `☞ heading` italic guides between subsections. The term "ego" leaves the user-facing chrome (DB schema and internals keep it). The page first shipped as `/narrativa`; renamed mid-session to avoid the page-name / section-name collision. **`/espelho` polish**: name bookplate at the top, pane title/subtitle flip (questions become titles, descriptive placas become subtitles), `soul-updated` shift retires, `many-conversations` threshold tightens to ≥3, Vivo glyph flips from `◌` to `✼`. **Look-inside rail** finally surfaces the active cena (bug pending since CV1.E11.S1). **Narrative tenants** get authored soul summaries + 3-5 inscriptions each (mix of citations + own principles); loader strips self-attribution so the bookplate doesn't echo. **Avatar dropdown restructure** into four groups with Unicode glyphs (▶ ◆ ◌ ⌂ ✱ ✦ ⚙ ☰ ↗); decision recorded to stay typographic over an icon-font lib. **Width unification**: portraits + identidade align with the system's 980px outer (with 720px inner reading column). **Three rounds of link audits** as read-by-default propagated. 1194/1194 tests passing (was 1097). **Parked**: per-cena model selection (carries from v0.23.0), ego-term audit on legacy `/map` chrome, lede-synthesis LLM fallback for thin briefings, manual visual smoke against narrative tenants.

**Maintenance (2026-05-04) — Look-inside rail now exposes the active cena.** The "Olhar Dentro" panel had been listing persona/organization/journey/mode but silently dropping the cena, even though `composedSnapshot.scene` had been part of the type since CV1.E11.S1. Two things were missing: (1) `buildRailState` never read `sessions.scene_id` into the snapshot — it called `composedSnapshot()` without the `sceneKey` arg, so even canonical sessions anchored to a cena rendered with `composed.scene = null`; (2) the rail had no `rail-composed-scene` row at all. Fix wires the session's scene through to the snapshot and adds a row between persona and organization (mirrors the prompt order — cena injects between persona and org/journey via briefing-in-compose). New i18n key `rail.sceneLine` ("scene: {key}" / "cena: {key}"), live-update parity in `chat.js`, and `scene: null` added to the fresh-session literal so the type stays complete. Two new tests in `tests/web.test.ts` (visible when session has cena, hidden when not) plus the existing fresh-session probe extended. 1099/1099 tests passing. Asset bump: `chat.js?v=look-inside-scene-1`. Unparks the second item from v0.23.0's parked list — `project_cena_model_per_scene.md` still on hold.

**v0.23.0 released — *The mirror has a face* (2026-05-04).** Bundles ~32 commits since v0.22.0 into one shipping unit. Major thread: **CV1.E12 (The Mirror page)** — three stories that turned `/` into the contemplative entry-point the brand always promised. S1 chrome inversion (logo points to `/espelho`, then to `/` after the URL swap; `▶ Iniciar` pill takes the operational role). S2 living synthesis (typed `MirrorState` from `server/mirror/synthesis.ts`; three depth panes Vivo/Estou/Sou with axis colors, glyphs, numeric tiles, links on every entity name). S3 Ímãs (pinned phrases — daily rotation + manual pin, top tile + RELEMBRE rotating surface in Vivo, full management at `/espelho/imas`). **URL swap**: `/` is the Espelho, operational home moves to `/inicio`, `/espelho` 301-redirects. **CV1.E11 follow-up**: Território splits out of Memórias. **Naming**: Mapa Cognitivo → Mapa Interior; Inscrições → Ímãs (mid-implementation, the magnet metaphor translates cleaner). **Data fixes**: cena response_mode/length propagate to sessions on start; org/journey/layer summaries are editable on the workshops; `/espelho` conversation count now uses the same eligibility filter as `/conversations`. **Big silent bug found**: hono/jsx HTML-escapes `<style>` children, killing every quoted font-family declaration system-wide (italic kept landing because font-style is a separate decl, masking the failure for months). `raw()` sweep across every inline `<style>` fixes typography that had been falling back to system sans. **Sou pane** ships with EB Garamond italic + warm-amber drop-cap (literary epigraph treatment for the soul summary). **Avatar dropdown** reorder: Início → Território → Mapa Interior → Memórias (drops Conversas + Ímãs, both still reachable). 1097/1097 tests passing (was 1022 at v0.22.0). **Parked**: per-cena model selection (`project_cena_model_per_scene.md`), and "Olhar Dentro" exposing the active scene as a prompt component (`project_olhar_dentro_missing_scene.md`).

Before that: **CV1.E12.S3 — Inscriptions on the mirror (2026-05-03).** S3 closed the epic same-day as it opened. People stick things to mirrors — mantras, citations, lines they want to come back to. The S2 page was synthesized; S3 added the user's intentional, hand-pinned voice on top. New `inscriptions` table + CRUD scoped by user_id. New `pickInscriptionForToday` resolves: most recent pin wins → otherwise deterministic daily rotation (djb2 hash of `userId + UTC ymd` mod active count) → otherwise null. Render block at top of `/espelho` (serif italic, curly quotes via CSS, em-dash + author below). Management page at `/espelho/inscricoes`: add form + per-row pin/unpin/edit/archive actions + `<details>`-collapsed archived band. No JS — every action is a tiny POST + redirect. Discrete footer link from `/espelho` (`#cbd5e0` text). 17 new tests across `tests/inscriptions.test.ts` (CRUD + picker, 16) and `tests/inscriptions-routes.test.ts` (route handlers + ownership 404s, 11). Suite at 1090/1090. CV1.E12 epic closed; updated index, decisions reference, and CV1 Depth index.

Before that: **CV1.E12.S1 + S2 shipped — `/espelho` is alive (2026-05-03).** Same-day from epic draft to working contemplative surface. **S1 — chrome inversion:** the `◆ Mirror Mind` logo now points to `/espelho` (the page that finally honors the brand name); a new `▶ Iniciar` pill takes over the operational role the logo used to carry. Avatar dropdown gains an `Início` text shortcut. No `Espelho` item in the menu — the logo IS the entry, on purpose. **S2 — living synthesis:** new `server/mirror/synthesis.ts` module composes a typed `MirrorState` from current data: a glance line (one sentence — soul orientation + dominant voice in last 7d + focus journey of the week), a "what shifted since last visit" pulse (uses new `users.last_mirror_visit_at` column), and three depth cards `Sou / Estou / Vivo`. Visual system landed in two passes — first textual, then refined: serif italic glance, color-coded cards per axis (amber Sou / teal Estou / plum Vivo), per-axis glyphs (`✦ ◉ ◌`), and **numeric tiles** that lift counts out of prose into a typographic moment (big number, small caps label, italic sub-line). Voice line cut from the Sou card (already in glance — anti-redundancy). Tile labels pluralize. Drill-down arrows removed; pane headings became links. **27 new tests** (`espelho-synthesis.test.ts` 22 + `espelho-routes.test.ts` 9 — minus the 4 placeholder asserts that S2 replaced). Suite at 1057/1057. **Next:** CV1.E12.S3 (inscriptions) — user-curated phrases pinned above the synthesis with daily rotation + manual pin.

Before that: **Território split + Mirror page epic drafted (2026-05-03).** Two threads landed in the same session. **(1) Território split shipped:** `/memorias` was conflating two timeframes — the lived record (history + future Library) and the present-active world (cenas, travessias, organizações). New `/territorio` route owns the entity cards; `/memorias` slims to Library + Histórico. Avatar menu adds Território between Cognitivo and Memórias. New i18n namespace `territorio.*`; `memoria.*` trimmed. Tests split (`tests/territorio-routes.test.ts` is new; `tests/memoria-routes.test.ts` rewritten for the slim page). 1026/1026 tests passing. Commit: `feat(territorio): split present-active world out of /memorias`. **(2) `/espelho` epic CV1.E12 drafted:** product-designer conversation surfaced that the `◆ Mirror Mind` logo is doing two incompatible jobs — brand mark and operational entry. Fix: flip the logo to point at a new contemplative `/espelho` page (synthesized self-portrait) and add a `▶ Iniciar` pill for the operational role. Three stories drafted — S1 chrome inversion + skeleton, S2 living synthesis (glance + pulse + depth), S3 inscriptions (user-pinned phrases with daily rotation + manual pin). Design locked in conversation: corridor-mirror metaphor (2-second glance + lingering depth), hybrid update model (Sou lazy, Estou + Vivo fresh), narrative voice in present tense (no timestamps, no numeric badges). Roadmap, [decisions.md](../project/decisions.md), and CV1 Depth index updated.

Before that: **CV1.E9.S6 — Soul Voice como alternativa session-level no Cast (2026-05-01).** New `sessions.voice` column ("alma" | NULL) with mutual exclusion enforced at the data layer: setting voice=alma clears session_personas in one transaction; adding a persona via /conversation/tag clears voice. Streaming pipeline reads voice and forces isAlma=true regardless of reception's is_self_moment verdict — every turn composes through Alma when the cast is in Alma mode. Header Cast renders a single ♔ avatar (warm-amber, `#b8956a`) instead of persona avatars when voice=alma; cast picker `+` carries Alma as first-class entry at the top of the panel with its own form posting to `/conversation/voice`. Per-turn `Enviar para…` overrides still work in both directions (persona on Alma cast, Alma on persona cast). Reception still runs on Alma turns — only is_self_moment becomes redundant; org/journey/mode/is_trivial signals still feed scope seeding and mode resolution. Glyph chosen to be visually distinct from the existing ◇ (persona) and ⌂ (org) and from the previous Alma marker `◈` (which the user flagged as resembling the mirror itself). 909/909 tests passing (was 891; +18 across `tests/session-voice.test.ts` + new web suite blocks for the route and SSR rendering). [Decision recorded.](../project/decisions.md)

Before that: **CV1.E10.S2 — Response length axis + Advanced header grouping (2026-05-01).** New session-level axis `response_length` ∈ `{brief, standard, full}` (null = auto), exposed via the expression pass with explicit "length is primary size constraint" instruction so combinations like `essayistic + brief` (bounded reflection) and `conversational + full` (long plain paragraph) actually compose. Header reorganized: the always-visible mode pill is now grouped with the new length control behind a collapsed `Avançado ▾` disclosure, defaulting to auto/auto and showing compact `<mode>/<length>` summary when configured. Mobile fixed-panel pattern carries through (panel anchored top-right, segmented stacked vertically). Migration is idempotent ALTER mirroring `response_mode`. 890/890 tests passing (was 870; +20 across `tests/response-length.test.ts`, expanded expression and web suites). Driven by real-use observation: essayistic responses ran longer than wanted on 01/May/2026; the user explicitly chose user-exposed control over silent recalibration. [Decision recorded.](../project/decisions.md)

Before that: **Maintenance (2026-04-30, follow-up) — scope bubble badge moved to transition rule.** The seed-when-empty gate fix above had a side effect: the existing pool-suppression rule on the bubble badge (which had been masked while junctions stayed empty) finally fired correctly, hiding the `⌂ org` / `↝ journey` badge on every reload after the seed turn. The user observed the seed turn lost its visual marker. Replaced pool-suppression with a transition rule (`decideScopeTransition` in `server/scope-transition.ts`): show the badge on the turn whose scope differs from the previous assistant turn's scope; suppress on continuation. Symmetric with persona's `newPersonasThisTurn`. Both SSR (`mirror.tsx`'s `computeBubbleSignatures`) and streaming (server-computed flag in routing event, rendered by `chat.js`) share the helper, so F5 and mid-turn agree. New helper `getLastAssistantScopeMeta` in `server/db/entries.ts` for the streaming side. 6 unit tests in `tests/scope-transition.test.ts` cover transition matrix; chat.js bumped to `v=scope-transition-1`. [Decision recorded.](../project/decisions.md)

**Maintenance (2026-04-30) — scope auto-seed gate is now symmetric across the three axes.** First-conversation-in-prod scenario (28/Apr/2026) surfaced the gap: turn 1 was a casual greeting that yielded no scope; turn 2's reception classified `o-espelho + software-zen`; bubble badges rendered correctly via entry meta but the header's Scope zone stayed empty because the junction-seeding gate for org/journey was `isFirstTurn`-strict. Persona seeding had always used `personasEmptyBefore` (seed-when-empty); the asymmetry was deliberate at CV1.E4.S4 but framed scope as "set at session start" too literally — "session start" isn't always turn 1 when turn 1 is a greeting. Policy now uniform across the three axes via a pure helper `decideScopeSeeding` (`server/scope-seed.ts`); 9 unit tests cover the gate matrix (empty pools, populated pools, alma/trivial/forced suppression on persona axis only). No-creep guarantee preserved (once seeded, reception never overrides; manual `+`/`×` is the only way to change). [Decision recorded.](../project/decisions.md) Two stale assertions in `tests/web.test.ts` updated alongside (`Cast`/`Elenco` → `Personas` after a prior i18n rename) — 862/862 tests passing.

Before that: **CV1.E10.S1 trivial turn elision — released as `v0.20.0` (2026-04-27).** New `is_trivial` axis on reception classifies pure courtesy/protocol turns (greetings, acknowledgments, casual pings) and routes them to a third compose path — `composeMinimalPrompt` — that returns only the adapter instruction. No identity, no persona, no scope, no behavior. The model receives the user's text and replies in its default voice. Mutually exclusive with `is_self_moment` at the prompt level (an apontamento is never trivial); enforced post-parse. Three weights now exist: heavy (Alma), medium (canonical), light (trivial). Briefing #5 satisfied across the spectrum. The token-economy epic (CV1.E10) opens with this story; future stories drafted (per-role behavior subsets, scope summary vs full briefing, adapter conditional).

Before: **CV1.E8.S1 LLM call logging — released as `v0.19.0` (2026-04-27).** Every model invocation across the pipeline (reception, main, expression, title, summary) now writes a row to `llm_calls` with the full system prompt, user message, response, tokens, cost, latency, session_id, entry_id, env, error. Admin UI at `/admin/llm-logs` surfaces the rows with filters (role, session, model, search-in-prompt), pagination, a toggle for the global `llm_logging_enabled` setting, cleanup actions (clear all + clear older than N days), and JSON/CSV export with RFC 4180 escaping. Detail view renders system_prompt and response in monospace `<pre>` blocks with `white-space: pre-wrap` so admin reads what was actually sent and received — no markdown render, no whitespace collapse. Bilingual chrome (en + pt-BR). Defensive throughout — `logLlmCall` never throws, never blocks the pipeline. The substrate the next week of Alma refinements needs: every "the response read flat" complaint can now be traced to the exact prompt that produced it.

Before: **CV1.E9 Soul Voice — released as `v0.18.0` (2026-04-27).** Five stories (S1 self/doctrine layer, S2 Soul Voice identity + prompt port, S3 is_self_moment reception axis + composer integration, S4 "Enviar Para…" UI + manual override, S5 calibration test set + smoke guide) all shipped + validated end-to-end against the live model. Six follow-up commits absorbed real-use issues from the smoke (fresh-session empty rail, Alma bubble badge, popover anchor, rail hot-update, empty-Composed hide, popover button styling, three-movement preamble depth, expression-pass skip on Alma turns, never-hang recovery, thinking-block fallback for reasoning-model empties, voice-neutral pronoun rule). The Alma now lands the canonical o-espelho voice work (acolher → iluminar → revelar) with doctrine principles applied not explained, in voz neutra sem pronomes diretos. *"Works for me"* milestone reached; Veronica is next. The mirror-mind now has a fundamentally different second voice — the Soul Voice — that engages on journal-tone fragments of personal weight, replaces the persona pipeline with a wise-voice composition (Alma preamble + soul + doctrine + identity), cites the user's own declared principles when they ressoam, and serves the *"Works for me, then works for Veronica"* adoption milestone. Manual override via "Enviar Para…" provides the escape hatch + ground-truth labels for future calibration. The o-espelho's conversational prompt (szen_play `prompts/services/travessia_entry_chat.md`) was the source — port adapted to be generic across users (doctrine slot is optional). 784/784 tests passing (was 729 at CV2.E1 close; +55 new across S1, S2, S3, S4, S5). Manual smoke roteiro lives in [cv1-e9-s5-calibration/test-guide.md](../project/roadmap/cv1-depth/cv1-e9-voz-da-alma/cv1-e9-s5-calibration/test-guide.md) — needs to run before the epic is marked closed.

Before: **CV2.E1 Localization fully shipped (2026-04-27).** All five user-facing stories (S1, S2, S3, S4, S5) plus S5b (Bia Lima) plus S2b (admin chrome) are done. The full app — sidebar, login, home, /me, /conversation surface, /conversations, /organizations, /journeys, /map, layer workshops, /personas, /docs nav, /admin dashboard, /admin/{users, budget, models, oauth} — renders in pt-BR for tenants with `users.locale = 'pt-BR'`. Antonio Castro promoted to admin (Brazilian household host, symmetric with Dan Reilly's role in the Reilly–Marchetti household). Test pollution bug fixed: narrative-loader tests now write tokens to `os.tmpdir()` paths via the new `tokensPath?` option; production `.tokens.local` no longer tampered with by `npm test`. 729/729 (was 727).

Before: **CV2.E1 Localization — S1, S2 (user-facing), S3, S4, S5 + S5b shipped; only S2b (admin) deferred** (2026-04-27). The Brazilian household now has two cohabiting tenants: **Antonio Castro** (creator/educator, mineiro in Floripa) and **Bia Lima** (his wife — pediatric emergency physician, mineira from Sete Lagoas). The son Tonico was aged up from 7 to 12 — Antonio's `father` persona, the `tonico-cresce` journey, and conversation 04 were rewritten to reflect a pre-adolescent with a phone, an Instagram he uses to look his father up, and the first sustained-under-pressure lies. The two tenants are isolated at the DB layer but share narrative events: the night Bia came home exhausted and Antonio sat behind a half-closed office door appears in both — as a `husband` conversation in Antonio (he names what he was avoiding and goes to her) and as an `esposa` conversation in Bia (she chooses an escalation in three frames over three weeks rather than a marker DR). 4 new tests in `narrative-loader.test.ts` (Bia locale=pt-BR, identity layers, orgs+journeys+conversations, isolation between Antonio and Bia). 727/727 (was 723). Bia covers a register the family didn't reach: emergency-decision medical, adult-mother of a pre-adolescent, friendship-at-distance with two college friends in BH/SP, an open senior-clinical promotion in the hospital that she ends up declining for a sound reason — no romance about it.

Before: **CV2.E1.S5 — Antonio Castro tenant** (2026-04-27). The Brazilian tenant is real: **Antonio Castro**, mineiro-em-Floripa, creator/educador independente, casado com Bia (pediatra), pai do Tonico (7), três Antônios numa linhagem mineira. Sua pasta em `docs/product-use-narrative/users/antonio-castro/` carrega 23 arquivos: profile, index, identity (self/soul + ego × 3 + 5 personas: creator/writer/father/husband/son), 2 organizações (pages-inteiras, lagoa-letras), 5 travessias (o-livro, voltar-a-bh, pos-lancamento, bia-saturada, tonico-cresce), 5 conversas canônicas. O loader (`narrative-loader.ts`) ganha leitura de frontmatter `locale: pt-BR` no `profile.md` e persiste em `users.locale` via UPDATE pós-createUser. Cobertura: novo arquivo `tests/narrative-loader.test.ts` com 8 probes — antonio-castro tem locale=pt-BR, os 4 Reilly/Marchetti permanecem en, todas as 9 layers de identity carregam, ambas orgs, todas 5 journeys, sessions importadas, link pos-lancamento → pages-inteiras, idempotência. **Apenas S2b (admin)** continua aberta. 723/723 (era 715; +8 novos).
...
- Veronica's tenant + onboarding (out of CV1.E9 scope; opens once Alisson confirms voice).

**CV1.E7 — Response Intelligence:**
- **S2b — Reception calibration** — revisit reception's classification rules if real use surfaces mis-classification on any axis (mode, persona under constrained pool, identity-touching detection, S8 suggestion frequency, **CV1.E9.S3 is_self_moment**).
...
The dor that drove this surfaced on day-one of Alma usage: opening a session with "boa noite" was sending the entire ~5KB `ego/behavior` block (Pensamento + Estrutura + Postura + Condutas + Exemplo de voz) just to receive a one-line greeting back. Multiplied by every conversation that opens or closes with a courtesy, the structural waste was real.

**Shipped across 7 phases:**

1. **Story docs** — `docs/.../cv1-e10-token-economy/cv1-e10-s1-trivial-elision/{index,plan,test-guide}.md`. Epic `cv1-e10-token-economy/` opened with this S1 plus three drafted future stories (per-role behavior subsets, scope summary vs full briefing, adapter conditional).
2. **Reception axis** — new `is_trivial: boolean` on `ReceptionResult`. NULL_RESULT carries `false`. System prompt extension with three positive classes (greetings / acknowledgments / casual pings), border cases that should NOT flip true (apontamentos, ack+affirmation, greeting+ask), and explicit mutual-exclusion rule with `is_self_moment`. Strict parser; mutual-exclusion enforced post-parse (if both true, force is_trivial false — Alma wins). 10 new tests in `tests/reception.test.ts`.
3. **Minimal composer** — new `composeMinimalPrompt(adapter?)` exported from `server/identity.ts`. Pure function of the adapter parameter (no DB), returns the adapter instruction or empty string. 4 new tests in `tests/identity.test.ts`.
4. **Snapshot extension** — `composedSnapshot` gains `isTrivial?: boolean = false`. When true: layers empty, personas empty, scopes null, isAlma forced false. The existing fresh-session-empty-Composed-block CSS hides the rail's section automatically — no rail UI changes. 3 new tests.
5. **Pipeline routing** — all 3 adapters (web stream, web sync, telegram) gain the branch `isTrivial → composeMinimalPrompt; isAlma → composeAlmaPrompt; else → composeSystemPrompt`. Persona pool seeding skipped on trivial turns. `_is_trivial: true` stamped on assistant entry meta for F5 reload. `_touches_identity` forced false on trivial turns regardless of reception's verdict. `buildRailState` extended with `overrideIsTrivial` parameter; derives from `_is_trivial` meta on F5 reload.
6. **Calibration probes** — `tests/voz-da-alma-calibration.test.ts` extended with 9 new cases (6 trivial + 3 border cases that should NOT be trivial). Total calibration set: 27 cases. Existing `edge-greeting` updated to assert `expectedIsTrivial: true`.
7. **Wrap-up** — worklog, decisions, badges, version bump to 0.20.0, tag.

**Tests: 847 passing** (was 821 at v0.19.0; +26 new across reception axis, minimal composer, snapshot, calibration probes).

**Decisions installed:**
- Three compose paths form the symmetric weight spectrum: minimal (CV1.E10.S1), canonical, alma (CV1.E9.S2). Each gated by an explicit reception axis (or absence of axes for canonical). Mutually exclusive at trigger level; mutual-exclusion enforced post-parse for defensive belt-and-suspenders.
- Trivial path is intentionally NOT zero-prompt — it keeps the adapter instruction (formatting cues for the channel). If real use surfaces "responses too generic", v2 adds a 30-token baseline ("you are the user's personal mirror; respond briefly and warmly in their language") instead of reverting to full behavior.
- No new UI — trivial turns simply produce an empty Composed snapshot, which the CV1.E9 follow-up's `data-empty="true"` CSS rule hides automatically. The empty Composed IS the visual signal that the turn was elided.

### 2026-04-27 — CV1.E8.S1 LLM call logging — released as `v0.19.0` ✅

The pipeline became inspectable. Every LLM invocation now persists its full system prompt, user message, and response — plus tokens, cost, latency, session/entry refs, env, error — to a new `llm_calls` table. Admin can read the prompts verbatim in monospace blocks, filter by role/session/model/search, export as JSON/CSV, toggle the whole capture off when not investigating, and clear old rows manually. The substrate the next week of Alma refinements needs: every "the response read flat" complaint traces to the exact prompt that produced it.

**Shipped across 8 phases:**

1. **Story docs** — `docs/.../cv1-e8-s1-llm-logging/{index,plan,test-guide}.md`.
2. **Schema + DB helpers** — new `llm_calls` table (id, role, provider, model, system_prompt, user_message, response, tokens_in/out, cost_usd, latency_ms, session_id, entry_id, user_id, env, error, created_at) with indexes on (role, created_at), (session_id, created_at), (created_at). New settings key `llm_logging_enabled` defaulting to true. Helpers in `server/db/llm-calls.ts`: insertLlmCall, setLlmCallEntryId, listLlmCalls (with role/session/model/date/search filters + pagination), countLlmCalls, getLlmCall, listLlmCallModels, deleteAllLlmCalls, deleteLlmCallsOlderThan. 18 tests in `tests/llm-calls.test.ts`.
3. **Logging service** — `server/llm-logging.ts` with `logLlmCall()` checking the toggle and writing the row, never throwing (catches every error and logs to stderr). `linkLlmCallEntry()` backfills entry_id after the assistant entry is appended (used by main pipeline's two-step write). 8 tests in `tests/llm-logging.test.ts`.
4. **Hook integration in 5 sites** — reception (extends ReceptionContext with sessionId; logs success + parse-failure + timeout paths), expression (logs single row per LLM invocation including the empty-response fallback case), title and summary (both functions in summary.ts), main generation in 3 adapters (web stream, web sync, telegram) — capturing latency around the call and logging AFTER the assistant entry is appended so entry_id populates immediately.
5. **Admin UI list page** — `/admin/llm-logs` with filters (role dropdown, session id input, model dropdown, search-in-prompt input), 50 rows per page, columns: timestamp, role badge (color-coded per role), model, tokens, cost, latency, session preview, error indicator. Pagination at the bottom. Detail page `/admin/llm-logs/:id` with full system_prompt + user_message + response in monospace `<pre>` blocks (dark theme with `white-space: pre-wrap`), metadata table beside.
6. **Toggle + cleanup actions** — POST /admin/llm-logs/toggle flips the global setting; POST /admin/llm-logs/clear deletes all rows; POST /admin/llm-logs/clear-older?days=N deletes rows older than the cutoff. Confirm dialogs on destructive actions.
7. **Export** — GET /admin/llm-logs/export?format=json|csv with current filters applied. JSON returns array of rows. CSV uses RFC 4180 quoting (every cell wrapped in double quotes, embedded quotes doubled) so multi-line system_prompt cells survive intact when opened in spreadsheets. Capped at 10k rows per export for memory bounds.
8. **Wrap-up** — i18n strings (~50 keys × 2 locales), worklog, decisions, badge flip, version bump to 0.19.0.

**Tests: 821 passing** (was 785 at v0.18.0; +36 new across `tests/llm-calls.test.ts` (18 helper), `tests/llm-logging.test.ts` (8 service), `tests/web.test.ts` (10 admin route smoke including detail rendering, filter narrowing, toggle, clear, JSON export, CSV escaping)).

**Decisions installed:**
- Logging defaults ON. The cost is the storage; the value is the next-week-of-iteration unblocked. Admins can flip OFF when investigating moves to a different surface.
- No automatic retention. Manual cleanup. The "important" calls (Alma turns the user wants to revisit) are heterogeneous; bias toward keeping until admin says otherwise.
- `llm_calls` lives alongside `usage_log`, not replacing it. usage_log stays cost-only for budget reconciliation queries; llm_calls carries prompts + responses for the inspection use case. Joinable on `(role, created_at)` proximity if needed; not a strict FK.
- Detail view renders prompts in monospace `<pre>` with `white-space: pre-wrap` and dark background. NO markdown render — admin needs to see what was actually sent (the bytes the model received), not how it would render.

### 2026-04-27 — CV1.E9 Soul Voice — released as `v0.18.0` ✅

The mirror-mind grew a second voice. The **Soul Voice** is a structurally Self composition path — engaged when reception flags a turn as a journal-tone fragment of personal weight (`is_self_moment: true`) — that REPLACES the persona pipeline. The Alma speaks from the user's center: a small framing preamble + the user's `self/soul` (essence) + `self/doctrine` (newly added in S1 — adopted framework / 9 Princípios for Alisson) + `ego/identity` (operational positioning) + `ego/behavior`, with optional scope context. Persona blocks are skipped — the Alma is the voice. The o-espelho conversational prompt in szen_play (`prompts/services/travessia_entry_chat.md`) was the source of the tone calibration; the port adapted it to be generic across users (doctrine slot is optional). 

A second send button in the chat — **"Enviar Para…"** — gives the user a manual destination picker (Soul Voice + every persona in inventory). Manual choices override reception, route the turn through the chosen destination, and persist as `_forced_destination` labels paired with reception's auto verdict. The pair becomes labeled training data for future calibration of the auto-detector.

**Shipped across 5 stories:**

1. **S1 — `self/doctrine` layer** ([story](../project/roadmap/cv1-depth/cv1-e9-voz-da-alma/cv1-e9-s1-doctrine-layer/)). New identity layer alongside `self/soul`. Composes between soul (essence) and identity (positioning) when reception flags identity-touching OR when the Alma engages. Same gating as the S4 identity cluster — empty doctrine row silently skips. The user's adopted framework (principles, doctrines, mental models) lives here. Alisson's content (the 9 Princípios da Liderança Soberana) lives in `docs/seed/alisson/doctrine.md` and seeds via the admin command. The `getIdentityLayers` SQL ordering was extended so soul precedes doctrine within the self namespace; the snapshot's `isIdentity` filter was extended to gate doctrine alongside soul + identity.

2. **S2 — Soul Voice identity + prompt port** ([story](../project/roadmap/cv1-depth/cv1-e9-voz-da-alma/cv1-e9-s2-alma-prompt/)). New `server/voz-da-alma.ts` exports `composeAlmaPrompt(db, userId, scopes, adapter?)` — sibling to `composeSystemPrompt`. The Alma path always composes the identity cluster (bypasses reception's identity gate), skips persona blocks, composes scope when reception activated it, and prepends the `ALMA_PREAMBLE` — a generic-across-users framing block that establishes voice (Quiet Luxury, sereno-elevado-acolhedor, autoridade serena), posture (the user is sharing a registry of life — your reply is a soft mirror back), and how to weave the layered identity material below. The composed snapshot gained an `isAlma: boolean` field; when true, personas are forced empty and identity layers are forced visible regardless of the includeIdentity caller arg.

3. **S3 — `is_self_moment` reception axis + composer integration** ([story](../project/roadmap/cv1-depth/cv1-e9-voz-da-alma/cv1-e9-s3-is-self-moment/)). New boolean axis on `ReceptionResult`. NULL_RESULT carries `false`. Reception system prompt extended with the three-class taxonomy (Apontamento de vida → true; Pergunta funcional → false; Reflexão analítica sem peso pessoal → false), 6-8 examples per class, form signals for and against, and the conservative-by-default framing (false positives are corrosive; false negatives are recoverable via S4's manual override). Parser is strict on literal `true` only — anything else lands at `false`. All three pipeline adapters (web stream, web sync API, telegram) branch the composer choice on `reception.is_self_moment`, skip persona pool seeding on Alma turns, force identity-on for Alma turns, and stamp `_is_alma`/`_touches_identity` on the assistant entry meta. The rail's `composedSnapshot` plumbing accepts and propagates `isAlma` through `buildRailState`. The Look inside snapshot in the rail now renders a `◈ Soul Voice` indicator when the turn was Alma-routed.

4. **S4 — "Enviar Para…" UI + manual override** ([story](../project/roadmap/cv1-depth/cv1-e9-voz-da-alma/cv1-e9-s4-enviar-para/)). Backend extension: `/conversation/stream` (and `/api/message`) accept a `forced_destination` parameter (`alma` or `persona:<key>`). When present, the override wins over reception's persona/Alma routing for the turn — reception still runs (we want mode/scope/identity axes) but its picks are replaced where overridden. The assistant entry meta carries `_forced_destination` (canonical "what was used") plus `_reception_is_self_moment` and `_reception_personas` (cross-reference with reception's auto verdict — the labeled-data pair). UI: a second button in the chat form (`#send-to-btn` next to Send) anchored to a popover (`#send-to-popover`) listing Soul Voice + each persona in the cast (cast members first, then non-cast inventory alphabetical). `chat.js` factored the existing submit handler into `runSend(text, forced)` so both the canonical send and the popover-routed send share the same flow. ESC and click-outside close the popover. Both buttons disable mid-stream.

5. **S5 — Calibration with Antonio/Bia diary entries** ([story](../project/roadmap/cv1-depth/cv1-e9-voz-da-alma/cv1-e9-s5-calibration/)). New `tests/voz-da-alma-calibration.test.ts` — 18-case contract probe across the three classes (6 apontamento, 4 functional, 4 analytical, 4 edge). Each case mocks the LLM verdict and asserts the parser preserves it — catches accidental parser drift without requiring a live LLM call. Live-LLM classification quality is validated by the manual smoke roteiro in [test-guide.md](../project/roadmap/cv1-depth/cv1-e9-voz-da-alma/cv1-e9-s5-calibration/test-guide.md) — seven tests covering casual greeting (no Alma), apontamento (Alma engages), functional question (canonical persona path), analytical reflection (canonical path — the trap), forced Alma override on functional message, forced persona override on apontamento, and F5 reload reproducing the routing.

**Tests: 784/784 passing** (was 729 at CV2.E1 close). +55 new across `tests/identity.test.ts` (5 doctrine), `tests/composed-snapshot.test.ts` (3 doctrine), `tests/voz-da-alma.test.ts` (15 alma composer + snapshot + override), `tests/reception.test.ts` (10 is_self_moment axis), `tests/voz-da-alma-calibration.test.ts` (19 contract probes including the 18 cases + 1 set-coverage smoke).

**What still needs to happen before the epic is marked ✅ shipped (instead of in-progress):**

1. Run the manual smoke roteiro end-to-end against the live model. The contract tests pass; live-classification quality is the open variable.
2. Seed Alisson's `self/doctrine` via the admin command.
3. If smoke surfaces consistent mis-classification, iterate the `is_self_moment` block in `server/reception.ts`.
4. Update the roadmap status badges across `cv1-e9.../index.md` and the per-story `index.md` files from 🚧 to ✅ once smoke clears.

**Decisions installed:**
- `self/doctrine` is its own layer (not bundled into `self/soul`) — option 2 of the trio considered. Future-proofs the case where a user adopts a framework different from another user's; doesn't pollute the soul of the existing narrative tenants who don't have a doctrine.
- Soul Voice is core, not extension. Auto-detection ships from day 1 (S3); manual override (S4) is the escape + labeled-data feedback loop, not the primary path.
- Voice naming asymmetry: internal code says "self" (`is_self_moment`, `composeAlmaPrompt`); user-facing UX says "Soul Voice" (or "Alma"). Junguian terminology stays internal; external language is accessible.
- Expression pass stays on for Alma turns (mode-aware shaping; preserve-substance rules don't conflict). If smoke surfaces tone drift, S5 calibration adds an Alma-specific bypass.
