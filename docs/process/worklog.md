...
Current focus: **CV2.E1.S1 — i18n infrastructure** just shipped. New epic CV2.E1 (Localization) opened ahead of channel work to unblock non-English-speaking users — the immediate motivation is onboarding a Brazilian family member; the broader payoff is that a UI layer can now travel without depending on D7's English-internal mandate. S1 ships the rail only: tiny `t(key, locale, params?)` with a fallback chain (locale → en → crude key + warn), two empty resource files (`adapters/web/locales/{en,pt-BR}.json`), and a Hono middleware that resolves the per-request locale from `user.locale` (S3 schema column, defensively read until S3 lands) or `Accept-Language`. Registered on the `web` sub-app after `webAuthMiddleware`; login stays unlocalized in S1. No copy changed — production renders identical. 707 tests passing (was 689; +18: 12 unit on `t()`, 6 smoke on the middleware via `app.request()`). Decision recorded: [user-facing locale ≠ D7](../project/decisions.md). Follow-on stories drafted in [CV2.E1 index](../project/roadmap/cv2-accessibility/cv2-e1-localization/) (S2 externalize → S3 user pref → S4 pt-BR fill → S5 Brazilian narrative tenant).
